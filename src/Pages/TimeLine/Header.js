import { useGetTimeLineImgsQuery, usePostTimelineImgMutation } from 'api/apiSlice'
...
function TimeLineHeader () {
      ...
      <div>
        {
          ['Reject Request', 'Accept Request'].map(label => (
            <Badge key={label} onMouseDown={(e) => frndUserRelationChange(frnd, label, e)} className="cursor-pointer mx-2" text="dark">
              {label}
            </Badge>
          ))
        }
  ...
  const postTimelineImage = () => {
    ...
    if (supportedImgFormats.includes(name.split('.').pop().toLowerCase())) {
      ...
      postImage({ file: postedImg, caption: postedImgCaption, username: currentUser() })
        .unwrap()
        .then(() => closeModal())
      ...
      >
        ...
        />
          <Dropdown.Menu show={openProfileDrpDwn} align="end" style={{ top: 60, right: 50 }}>
            <Dropdown.Item
              onClick={() => {
                const view = timelineState !== 'FRIEND_LIST_VIEW' && timelineState !== 'MESSAGE_VIEW' ? 'FRIEND_LIST_VIEW' : 'TIMELINE_LOADED'
                switchViews(view)
              }}
            >
